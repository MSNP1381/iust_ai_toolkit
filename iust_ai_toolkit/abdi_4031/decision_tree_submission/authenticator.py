...
class DecisionTreeAuthenticator(BaseAuthenticator):
    def __init__(self, base_dir: str = None):
        ...
        self.ignore_cheating_percentage = 0.7  # Ignore cheating if this percentage of students are classified as cheating
    ...
    def compare_submissions(self, student_id1: str, student_id2: str) -> Tuple[float, Dict]:
        if not self.is_ta_version_installed():
            raise ImportError("TA version is not installed. Please install iust_ai_toolkit[ta] to use this feature.")
        ...
        verbose_result = {
            ...
            'common_methods': list(set(data1['implemented_methods']) & set(data2['implemented_methods'])),
            'unique_methods_1': list(set(data1['implemented_methods']) - set(data2['implemented_methods'])),
            'unique_methods_2': list(set(data2['implemented_methods']) - set(data1['implemented_methods'])),
            'common_estimations': {k: (data1['estimations'][k], data2['estimations'][k]) for k in estimation_keys},
        }
    ...
    def analyze_all_submissions(self, submissions: List[str]) -> Tuple[List[Tuple[str, str, float]], Dict[str, Dict], bool]:
        results = []
        verbose_results = {}
        ...
        cheating_percentage = potential_cheating_count / total_comparisons if total_comparisons > 0 else 0
